...
def gemini_best_match(user_input):
    prompt = (
        "Tu es ODIN, un assistant RH automatisé.\n"
        "Voici une base de données de questions et réponses fréquentes :\n\n"
    )
    for q, a in QaN_GENERAL.items():
        prompt += f"{a}\n\n"
    
    prompt += (
        "---\n\n"
        f"Réponds directement et clairement à cette question, même si elle n'est pas identique :\n\"{user_input}\""
    )
...
while True:
    ...
    if user_input.lower() in ["exit", "quitter"]:
        type_writer("\nODIN : Merci, à bientôt ! 👋")
        break

    # 🔍 1. Try exact or dictionary match first
    response = get_best_match(user_input)

    if not get_best_match(user_input):
        print(random.choices(f"{GEM_101}"))

    # 🤖 2. If no response found, try OpenAI
    if not response:
        response = gemini_best_match(user_input)

    # 🧠 3. Still nothing? Try deeper logic
    if not response:
        response = handle_complex_case(user_input)

    # 📣 4. Show the final response (default fallback is built into AI or handle_complex_case)
    type_writer(f"\n🤖 ODIN : {response}")

    # 🔁 5. Loop message
    type_writer(random.choice(LOOP_101))
